...
template<class T> tbuffer<T>::tbuffer(int size)
{
    // Save buffer size
    bufferSize = size;
    // Create semaphores
...
template<class T> T tbuffer<T>::get()
{
    // Local variable(s)
    T data;
    // Acquire semaphores
...
template<class T> bool tbuffer<T>::clear()
{
...
template<class T> int tbuffer<T>::maxSize()
{
    return bufferSize;
}

template<class T> bool tbuffer<T>::isFull()
{
    return queue.size()==bufferSize;
}

template<class T> bool tbuffer<T>::isEmpty()
{
    return queue.size()==0;
}
