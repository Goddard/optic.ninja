...
CameraView::CameraView(QWidget *parent, int deviceNumber, bufferThread *sharedImageBuffer) :
    QWidget(parent),
...
CameraView::~CameraView()
{
    if(isCameraConnected)
    {
        ...
        // Disconnect camera
        if(capturingThread->disconnectCamera())
...
bool CameraView::connectToCamera(bool dropFrameIfBufferFull, int capThreadPrio, int procThreadPrio, bool enableFrameProcessing, int width, int height, QString videoFile)
{
    ...
    if(capturingThread->connectToCamera(videoFile))
    {
        ...
        connect(capturingThread, SIGNAL(updateStatisticsInGUI(struct ThreadStatisticsData)), this, SLOT(updateCaptureThreadStats(struct ThreadStatisticsData)));
        ...
        capturingThread->start((QThread::Priority)capThreadPrio);
        ...
        ui->cameraResolutionLabel->setText(QString::number(capturingThread->getInputSourceWidth())+QString("x")+QString::number(capturingThread->getInputSourceHeight()));
